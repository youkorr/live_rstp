...
jpeg_decoder_config_t jpeg_config = {
.output_format = JPEG_DECODE_OUT_FORMAT_RGB565,
.rotate = JPEG_ROTATE_0D,
};

```
ret = jpeg_new_decoder_engine(&jpeg_config, &jpeg_decoder_);
if (ret != ESP_OK) {
...
esp_h264_dec_config_t h264_config = {
.max_width = 1920,
.max_height = 1080,
...
ret = jpeg_decoder_process(jpeg_decoder_,
...
ret = jpeg_decoder_get_info(data, len, &info);
if (ret == ESP_OK) {
...
size_t bytes_read = client_->readBytes(buffer_.get(), available);
...
// (logique simplifiée, il faudrait implémenter la défragmentation RTP complète)
...
// Implémentation des méthodes RTSP (identique à la version précédente)
bool LiveComponent::connect_rtsp() {
// [Code RTSP identique à la version précédente]
// … (pour économiser l’espace, pas de changement dans cette partie)
